...
class MPNSTDataset(Dataset):
    def __init__(
        self, data, transform=None, target="MPNST", mri_type="T1", is_train=True, ds_type="forgot", do_load=True
    ):
        self.target = target
        self.data = data
        self.type = mri_type
    ...
    def _prepare_biggest_images(self):
        big_image_indexes = {}
        if (f"big_image_indexes_{self.ds_type}.pkl" in os.listdir(f"input/{self.type}"))\
            and (self.do_load) :
            ...
            for row in tqdm(self.data.iterrows(), total=len(self.data)):
                ...
                path = f"/trinity/home/xwan/data/MPNST/{case_id}/{self.type}.nii.gz"
                files = sorted(
                    glob.glob(path),
                    key=lambda var: [
